...
export default async function ServicesPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-secondary">Definisci i servizi prenotabili, durata e prezzo.</p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-card border border-border border-l-4 border-l-teal bg-surface shadow-card p-5">
          ...
          <form action={addService} className="mt-4 space-y-3">
            ...
            <textarea name="description" placeholder="Descrizione" rows={2} className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-brand focus:outline-none" />
            <div className="flex gap-2">
              <input name="duration_minutes" type="number" min={5} step={5} defaultValue={30} placeholder="Durata (min)" className="w-1/2 rounded-lg border border-border px-3 py-2 text-sm focus:border-brand focus:outline-none" />
              <input name="price" type="number" min={0} step={0.5} placeholder="Prezzo (€)" className="w-1/2 rounded-lg border border-border px-3 py-2 text-sm focus:border-brand focus:outline-none" />
            </div>
            ...
            <button className="w-full rounded-button bg-brand py-2 text-sm font-semibold text-white hover:bg-brand-dark">Aggiungi prestazione</button>
          </form>
        </div>
      </div>
    </div>
