    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-3">
          {(articles || []).map((a) => (
            <div key={a.id} className="rounded-card border border-border border-l-4 border-l-teal bg-surface shadow-card p-4">
              ...
              {a.excerpt && <p className="mt-1 text-sm text-secondary">{a.excerpt}</p>}
              <div className="mt-3 flex items-center gap-3">
                {a.status === "draft" && <PublishButton articleId={a.id} />}
                {a.auto_publish_social && <span className="text-xs text-muted">Auto-pubblicazione social attiva</span>}
              </div>
            </div>
          ))}
        ...
        <form action={createArticle} className="space-y-3 rounded-card border border-border border-l-4 border-l-teal bg-surface shadow-card p-5">
          ...
          <textarea name="content" required rows={6} placeholder="Contenuto dell'articolo…" className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-brand focus:outline-none" />
          ...
          <button className="w-full rounded-button bg-brand py-2 text-sm font-semibold text-white hover:bg-brand-dark">Salva articolo</button>
        </form>
      </div>
      ...
      <section>
        ...
        <div className="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <form action={saveSocialAccount} className="space-y-3 rounded-card border border-border border-l-4 border-l-teal bg-surface shadow-card p-5">
            ...
            <input name="access_token" defaultValue={igAccount?.access_token} placeholder="Access Token" className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-brand focus:outline-none" />
            <button className="w-full rounded-button border border-brand py-2 text-sm font-semibold text-brand hover:bg-brand-light">Salva</button>
          </form>
        </div>
      </section>
