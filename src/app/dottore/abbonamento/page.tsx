...
export default async function SubscriptionPage({
  ...
  return (
    <div>
      ...
      {success && <p className="mt-4 rounded-lg bg-green-50 px-4 py-2 text-sm text-green-700">Abbonamento attivato con successo!</p>}

      <div className="mt-4 rounded-xl border border-black/10 bg-white p-5">
        <p className="text-sm text-black/60">Piano attuale</p>
        <p className="text-lg font-semibold">{PLAN_LABEL[doctor?.plan || "trial"]}</p>
        <p className="text-sm text-black/60">Stato: {STATUS_LABEL[doctor?.subscription_status || "inactive"]}</p>
      </div>

      <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div className="rounded-xl border border-black/10 bg-white p-6">
          <h2 className="font-semibold">Starter</h2>
          <p className="mt-1 text-sm text-black/60">Agenda, CRM pazienti, promemoria email.</p>
        ...
        <div className="rounded-xl border-2 border-brand bg-white p-6">
          ...
          <p className="mt-1 text-sm text-black/60">Tutto Starter + pagamenti online e pubblicazione social automatica.</p>
